...
function getTabIcon(routeName, focused, color, size) {
    ...
    if (routeName === 'Home')    return <Ionicons name={focused ? 'home' : 'home-outline'} size={s} color={color} />;
    if (routeName === 'Map')     return <Ionicons name={focused ? 'map' : 'map-outline'} size={s} color={color} />;
    if (routeName === 'Saved')   return <Ionicons name={focused ? 'bookmark' : 'bookmark-outline'} size={s} color={color} />;
    if (routeName === 'Profile') return <Feather name="user" size={s} color={color} />;
    ...
    return (
        <Tab.Navigator
            screenOptions={({ route }) => ({
                ...
                tabBarIcon: ({ color, size, focused }) => getTabIcon(route.name, focused, color, size),
            })}
        >
